...
async def habilitar_fuente(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [[InlineKeyboardButton(nombre, callback_data=f"habilitar_{nombre}") for nombre in bot.fuentes.keys()]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Selecciona la fuente a habilitar:", reply_markup=reply_markup)

async def deshabilitar_fuente(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    bot = context.bot_data['bot']
    if str(update.effective_user.id) != bot.config.USER_ID:
        await update.message.reply_text("No tienes permiso para usar este comando.")
        return

    keyboard = [[InlineKeyboardButton(nombre, callback_data=f"deshabilitar_{nombre}") for nombre in bot.fuentes.keys()]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Selecciona la fuente a deshabilitar:", reply_markup=reply_markup)

async def manejar_callback_fuente(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    bot = context.bot_data['bot']
