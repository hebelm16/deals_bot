...
class DealsnewsScraper(BaseScraper):
    ...
    def obtener_ofertas(self) -> List[Dict[str, Any]]:
        ...
        for i, seccion in enumerate(secciones_oferta):
            logging.debug(f"Procesando sección de oferta {i+1}")
            if seccion.find('div', class_='details-container'):
                try:
                    ...
                    oferta['titulo'] = titulo.text.strip() if titulo else 'No disponible'
                    
                    precio_elem = seccion.find('div', class_='callout limit-height limit-height-large-1 limit-height-small-1')
                    oferta['precio'] = precio_elem.text.strip() if precio_elem else 'No disponible'
                    ...
                    oferta['info_cupon'] = cupon_elem.text.strip() if cupon_elem else None
                    oferta['cupon'] = None
                    ...
                    logging.debug(f"DealNews - Oferta procesada: {oferta['titulo']}")
                except Exception as e:
                    logging.error(f"Error al procesar una oferta de DealNews: {e}", exc_info=True)
                    continue
            else:
                logging.debug(f"La sección {i+1} no contiene una oferta válida")
